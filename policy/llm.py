	...
	def progress2text(self):
		...
		if self.task == 'wind':
			...
			for obj, desc in zip(self.object_list, object_location_description_list):
				if obj['category'] != 'shopping cart':
					continue
				ps += f"name: {obj['category']}, id: {obj['id']}, distance: {desc}\n"
		else:
			ps = ""
		...
		for obj, desc in zip(self.object_list, object_location_description_list):
			...
			if obj['category'] not in self.target_objects or obj['id'] not in self.current_seen_objects_id:
				continue
			ps += f"name: {obj['category']}, id: {obj['id']}, value: {self.objects_info[obj['category']]['value']}, distance: {desc}, "
			if self.task == 'fire':
				...
				if obj['id'] not in self.env_change_record:
					...
					ps += f"water level: {self.env_change_record[obj['id']][-1]} m\n"
			else:
				ps += f"status: {'Unknown'}\n"
	...
	def get_available_plans(self):
		...
		if len(self.holding_objects) == 0:
			for obj in self.object_list:
				if obj['category'] not in self.target_objects or (len(self.holding_objects) > 0 and obj['id'] == self.holding_objects[0]['id']):
					continue
